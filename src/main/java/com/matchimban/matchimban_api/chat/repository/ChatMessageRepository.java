...
public interface ChatMessageRepository extends JpaRepository<ChatMessage, Long> {
	...
	@Query("""
		...
		where cm.meeting.id = :meetingId
		  ...
		  and (:cursor is null or CAST(cm.id AS string) < :cursor)
		order by cm.id desc
	""")
	...
	@Query("""
		select CAST(cm.id AS string)
		from ChatMessage cm
		where cm.meeting.id = :meetingId
		  and cm.isDeleted = false
		order by cm.id desc
	""")
	List<String> findRecentMessageIds(
		@Param("meetingId") Long meetingId,
		Pageable pageable
	);
	...
	@Query("""
		select (count(cm) > 0)
		from ChatMessage cm
		where CAST(cm.id AS string) = :messageId
	...
	@Query("""
		...
		where cm.meeting.id = :meetingId
		  ...
		  and CAST(cm.id AS string) > coalesce((
